...
export default function MyLife({data,today,navigate,toggleTask,Card,SectionTitle}){
  ...
  return <div className="my-life space-y-5">
    ...
    <div className="grid gap-5 xl:grid-cols-[.82fr_1.18fr]">
      ...
      <Card className="today-timeline-card"><SectionTitle icon={Clock3} title="Өнөөдрийн хуваарь" sub="Календарь, ажлууд, төлөвлөгөөний санал, дадал болон хоол"/>{plan.timeline.length?<div className="timeline-list">{plan.timeline.map((item,i)=><div className="timeline-row" key={item.id}><time className="timeline-time" dateTime={item.time}>{item.time}</time><div className={`timeline-dot timeline-${item.kind}`}/><div className="timeline-content"><b className="timeline-title">{item.title}</b><span className="timeline-meta">{timelineCategory[item.kind]||item.kind}{item.duration?` · ${item.duration} мин`:""}</span></div>{i<plan.timeline.length-1&&<i className="timeline-line"/>}</div>)}</div>:<div className="soft-empty">Одоогоор цагтай зүйл алга. Өнөөдрийн хуваариа бүрдүүлэхийн тулд ажил, үйл явдал, дадал эсвэл хоол нэмээрэй.</div>}</Card>
    </div>
